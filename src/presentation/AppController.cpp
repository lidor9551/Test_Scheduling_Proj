...
AppController::AppController(QObject* parent)
    ...
    m_outputManager.setMoveValidator(
        [this](const ScheduleGenerationResult& tempScheduleCopy,
            const std::string& courseId,
            const Date& targetDate) -> bool {

            const std::vector<SchedulingBlock>& blocks = this->m_allBlocks;
            ...
            for (const auto& block : blocks) {
                allowedDates.insert(
                    allowedDates.end(),
                    block.allowedDates.begin(),
                    block.allowedDates.end()
                );
            }

            MoveValidationResult result =
                this->m_moveValidationService.validateMove(
                    tempScheduleCopy,
                    courseId,
                    targetDate,
                    allowedDates,
                    blocks,
                    this->session_.getSettings()
                );
