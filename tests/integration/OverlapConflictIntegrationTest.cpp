...
int main() {
    const std::string basePath = "../tests/data/overlap/same-program-conflict/";
    ...
    EXPECT_EQ(block.runtimeCourses.size(), static_cast<std::size_t>(2));
    EXPECT_EQ(block.allowedDates.size(), static_cast<std::size_t>(1));
    ...
    std::cout << "OverlapConflictIntegrationTest passed." << std::endl;
    return EXIT_SUCCESS;
}