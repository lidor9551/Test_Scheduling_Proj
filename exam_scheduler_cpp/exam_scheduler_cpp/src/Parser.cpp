...
std::vector<Course> CourseFileParser::parse(const std::string& filePath) const {
    ...
    for (const std::string& record : records) {
        ...
        for (std::size_t i = 3; i + 1 < lines.size(); ++i) {
            ...
            std::string semester = Utils::trim(parts[2]);
            ...
            if (!Validator::isValidSemester(semester)) {
                throw ParseException("Invalid semester: " + semester);
            }
            ...
            offerings.push_back(ProgramOffering{programId, year, semester, requirement});
        }

        courses.emplace_back(name, number, instructor, offerings, evaluation);
    }

    return courses;
}

std::vector<ExamPeriod> ExamPeriodFileParser::parse(const std::string& filePath) const {
    const std::string text = Utils::trim(Utils::readFile(filePath));
    if (text.empty()) {
...
std::vector<std::string> SelectedProgramsFileParser::parse(const std::string& filePath) const {
    const std::string text = Utils::trim(Utils::readFile(filePath));
    if (text.empty()) {
