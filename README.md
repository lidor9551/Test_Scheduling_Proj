...
Shared courses first
Obligatory courses before elective courses
Courses with fewer possible dates first
...
The algorithm:
Selects the next course to schedule
Tries assigning it to each legal date
Checks whether the partial schedule is valid
Continues recursively if valid
Prunes the branch immediately if invalid
Saves the schedule when all courses are assigned
...
Filter only relevant courses with `Exam` evaluation
Keep only courses from selected study programs
Generate all legal dates from exam periods
Sort courses using priority heuristics
Assign dates using Backtracking
Check conflicts during assignment
Save every fully valid schedule
Write all valid schedules to the output file
